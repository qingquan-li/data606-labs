...
ggplot(samp1, aes(x = scientist_work)) +
  ...
  labs(
    x = "", y = "",
    title = "Sample distribution of scientist_work (n = 50)"
  ) +
  coord_flip()
```
...
The sample distribution follows the same general pattern as the population.

However, the exact sample proportions will not match the population proportions (80%/20%) precisely because of random sampling variability. With only 50 observations, we expect some deviation from the true population values, but the sample should be roughly close to the 80/20 split.
...
No, I would not expect another student's sample proportion to exactly match mine. Each student draws a different random sample of 50 people from the population, so the specific individuals selected will differ.

However, I would expect the proportions to be only somewhat different, not very different. Since both samples are drawn from the same population where the true proportion is 0.20, both sample proportions should hover around that value.
...
The sample proportion from `samp2` is likely close to, but not identical to, the sample proportion from `samp1`.

If we took samples of size 100 and 1000, the sample of size 1000 would provide a more accurate estimate of the population proportion. Larger samples reduce sampling variability.
...
```{r ex4-summary}
summary(sample_props50$p_hat)
```

There are 15,000 elements in `sample_props50`.

The sampling distribution is approximately normal (bell-shaped) and centered at around 0.20, which is the true population proportion. The distribution is roughly symmetric, with most sample proportions falling between about 0.10 and 0.30.
...
There are 25 observations in `sample_props_small`. Each observation represents the sample proportion of people who answered "Doesn't benefit" from one random sample of size 10 drawn from the population.
...
Each observation in the sampling distribution represents the proportion of "Doesn't benefit" responses from a single random sample of the given size.

As sample size increases from 10 to 50 to 100:
- the **mean** stays approximately the same at about 0.20 (the true population proportion), since the sample proportion is an unbiased estimator;
- the **standard error** decreases, meaning individual sample proportions cluster more tightly around 0.20;
- the **shape** becomes more symmetric and bell-shaped (more normal).

Increasing the number of simulations does not meaningfully change the mean or standard error of the sampling distribution. More simulations simply give us a smoother, more precise picture of the same underlying sampling distribution.
...
The best point is the sample proportion for the "Benefits" category from this sample.
...
ggplot(data = sample_props15, aes(x = p_hat)) +
  ...
  labs(
    x = "p_hat (Benefits)",
    title = "Sampling distribution of p_hat for Benefits",
...
The sampling distribution is roughly bell-shaped but somewhat left-skewed. It is centered near 0.80. Based on this sampling distribution, I would guess the true proportion is approximately 0.80.
...
The true population proportion of people who think the work scientists do enhances their lives is 0.80 (80,000 / 100,000).
...
```{r ex9-summary}
paste("Mean:", round(mean(sample_props150$p_hat), 4),
      "| SE:", round(sd(sample_props150$p_hat), 4))
```

The sampling distribution with n = 150 is more symmetric and more tightly concentrated around the center compared to the n = 15 distribution. It is approximately normal in shape.

Based on this sampling distribution, I would estimate the true proportion to be approximately 0.80, which is indeed the true population proportion.
...
The sampling distribution from sample size = 150 has a smaller spread than sample size = 15.

I would prefer a sampling distribution with a **small spread**. A smaller spread (i.e., a smaller standard error) means that the sample proportions are more tightly clustered around the true population proportion.
