...
A streak length of 1 means the player made one basket (H) and then missed the next shot (M). So there is 1 hit followed by 1 miss.
A streak length of 0 means the player missed a shot without making any baskets beforehand. It is just a single miss (M) with no preceding hit.
...
`calc_streak()` returns a numeric vector, not a data frame. To wrap it in a data frame. Replace it with:
...
The distribution of Kobe's streak lengths is right-skewed (skewed to the right). The majority of his streaks were short (0 or 1), with longer streaks becoming increasingly rare.

His typical (most frequent) streak length was 0.

His longest streak of consecutive baskets was 4.
...
The number of heads from the simulation is shown in the table above, that is 20, with the seed set to 12345.
...
Need to add the `prob` argument to specify that `"H"` (hit) has a probability of 0.45 and `"M"` (miss) has a probability of 0.55. Also, change `size` to 133 to match the number of shots Kobe took.
...
```{r exercise5}
sim_streak <- data.frame(length = calc_streak(kobe_basket$shot))
glimpse(sim_streak)
```
...
The distribution is also right-skewed, similar to Kobe's distribution.

The typical (most common) streak length is 0.

The longest streak is 4.
...
I would expect the streak distribution to be **somewhat similar**.

Since the simulation is based on random sampling, each run will produce different specific results due to random variation. However, the overall shape of the distribution should be similar, and it will still be right-skewed, the most common streak length will still be 0, and the typical and longest streaks should be in a similar range.

The general pattern is governed by the underlying probability (45%), but the exact counts and longest streak will vary from simulation to simulation due to chance.
...
Kobe Bryant's distribution of streak lengths is very similar to the distribution of the simulated independent shooter.

Both distributions are right-skewed with the most common streak length being 0. Kobe's longest streak was 4, which is within the range of what we would expect from an independent shooter with a 45% shooting percentage.

Since the simulated shooter's streak distribution generated under the assumption of independent shots, closely resembles Kobe's actual streak distribution, I don't have strong evidence that the hot hand model fits Kobe's shooting patterns. His streaks appear to be consistent with what I would expect from random chance alone, supporting the conclusion that successive shots are independent events.
