...
```{r ex1-sample-proportion}
samp %>%
  count(climate_change_affects) %>%
  mutate(p = n / sum(n))
```

63.3% (with `set.seed(42)`).
...
No, I would not expect another student's sample proportion to be identical to mine. Each student draws a different random sample from the population, so sampling variability will cause their proportions to differ.

I would expect the proportions to be *similar*. They should cluster around the true population proportion of 0.62.
...
"95% confidence" means that if we were to take many random samples from the population and construct a 95% confidence interval from each sample, we would expect approximately 95% of those intervals to contain the true population proportion.
...
# Check if 0.62 is captured
ci_lower <- as.numeric(ci[1])
ci_upper <- as.numeric(ci[2])
captured <- ci_lower < 0.62 & ci_upper > 0.62
captured
```

Yes, the true population proportion is 0.62. 

Most confidence intervals constructed from samples of this size will contain 0.62, but occasionally a sample will produce an interval that misses. A neighbor's interval may or may not capture the true value, depending on their particular sample.
...
I expect approximately 95% of those intervals to capture the true population proportion of 0.62. This is because we used a 95% confidence level, meaning the procedure is designed so that, in the long run, 95% of all intervals constructed this way will contain the true parameter.
...
0.96.
This proportion is likely close to 0.95 but not exactly equal to it. The reason is that 50 intervals is a relatively small number of trials, so we expect some random variation around the nominal 95% rate.
...
I will choose a 99% confidence level. I would expect a 99% confidence interval to be *wider* than the 95% confidence interval. A higher confidence level means we want to be more certain that our interval captures the true parameter, so we need to cast a wider net.
...
We are 99% confident that the true proportion of US adults who think climate change is affecting their local community falls between the lower and upper bounds reported above. This interval is wider than the 95% interval.
...
At the 99% confidence level, we expect approximately 99% of intervals to capture the true proportion. The proportion should be very close to (or exactly) 1.00 for 50 intervals, since it is rare for a 99% CI to miss. The wider intervals make it much more likely that each one captures the true value.
...
I will try a 90% confidence level. I expect this interval to be *narrower* than both the 95% and 99% intervals because a lower confidence level means we are accepting a higher chance of missing the true parameter in exchange for more precision.
...
We are 90% confident that the true proportion of US adults who think climate change affects their local community falls between the bounds shown above. As expected, this interval is narrower than the 95% and 99% intervals.
...
As sample size increases, the width of the confidence interval decreases. A larger sample provides more information about the population, which reduces sampling variability and thus reduces the standard error. Smaller samples produce wider intervals because there is more uncertainty about the true population proportion.
...
Increasing the number of bootstrap samples does not meaningfully change the width of the confidence interval. The standard error is a property of the original sample (it depends on the sample proportion and the sample size), not of how many bootstrap resamples we draw.
