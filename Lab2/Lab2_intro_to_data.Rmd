...
The three histograms show the same dep_delay data, but the appearance changes a lot depending on the bin width. The default and smaller bin width (`binwidth = 15`) reveal more detail (such as the concentration near small delays and some early departures), while the large bin width (`binwidth = 150`) hides important features and makes the distribution look much less informative.
...
Most flights arrive on time or early, with a smaller number of flights having large positive delays.
...
The carrier with the most variable arrival delays is the one with the largest IQR.
...
-   pros: to avoid rare but very bad delays, the mean may matter more.

-   cons: it can be heavily affected by a few extreme delays.

median

-   pros: more robust to extreme delays, so it may better reflect a “typical” departure experience.

-   cons: ignores extreme values (it does not utilize all data points in a distribution).
...
Based on on-time departure percentage, I would choose LGA (LaGuardia) because it has the highest on-time departure rate (72.8%) among the three NYC airports.
...
There is a positive relationship between `distance` and `avg_speed` at shorter distances: as distance increases, average speed tends to increase.\
This happens because takeoff/landing time affects shorter flights more. For longer flights, the speeds tend to level off and cluster in a narrower range.
...
ggplot(data = aa_dl_ua_flights, aes(x = dep_delay, y = arr_delay, color = carrier)) +
  geom_point()
```

From the plot, the rough cutoff is around 30 minutes of departure delay where flights can still often arrive on time (arr_delay <= 0).
