...
```{r plot-girls-vs-year-trend}
ggplot(data = arbuthnot, aes(x = year, y = girls)) +
  geom_line() +
  labs(
    title = "Number of Girls Baptized per Year",
    x = "Year",
...
**My finding:  
While there is noticeable variability from year to year, the long-term trend in the number of girls baptized appears to be increasing.**
...
```{r plot-proportion-of-boys}
ggplot(data = arbuthnot, aes(x = year, y = boy_ratio)) +
  geom_line()
```

**My finding:  
The proportion of boys is above 0.5 every year, which means that more boys are born than girls each year.**
...
```{r present-years-dimensions-column-names}
str(present)
range(present$year)
...
**My finding:  
Years: 1940-2002
Dimensions: 63 rows x 3 columns.
Variable (column) names: year, boys, and girls.**

1.  How do these counts compare to Arbuthnot's? Are they of a similar magnitude?

**Insert your answer here**

```{r compare-ranges-of-births}
arbuthnot %>%
  summarize(
    min_boys = min(boys),
    max_boys = max(boys)
...
**My finding:  
The birth counts in the present dataset are much larger than those in Arbuthnot’s data.  
Arbuthnot’s counts are in the thousands, while present-day U.S. birth counts are in the millions.**
...
```{r proportion-of-boys-born-over-time}
present <- present %>%
...
ggplot(data = present, aes(x = year, y = prop_boys)) +
  geom_line() +
  labs(
    title = "Proportion of Boys Born in the U.S. Over Time",
    x = "Year",
...
**My finding:  
The proportion of boys born each year remains consistently above 0.5, indicating that boys are born more frequently than girls.  
This supports Arbuthnot’s observation that boys tend to be born in greater proportion than girls, even in modern U.S. data.**
...
```{r year-with-the-most-total-births}
present %>%
  mutate(total = boys + girls) %>%
  arrange(desc(total))
```
...
**My finding:  
The year the most total number of births in the U.S. is 1961.**
